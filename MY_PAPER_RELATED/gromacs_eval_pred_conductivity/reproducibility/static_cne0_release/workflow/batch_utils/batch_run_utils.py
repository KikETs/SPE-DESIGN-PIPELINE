...
DEFAULT_CFG = {
    ...
    'production_dt_fs': 1.0,
    'production_ntomp': 12,
    'production_tcoupl': 'v-rescale',
    'production_tau_t': 5.0,
    'production_bonded_gpu': False,
    'production_lincs_iter': 6,
    'production_lincs_order': 12,
    'production_lincs_warnangle': 20,
    'lincs_abort_warnings': 25,
...
def ensure_pipeline_script(config: GromacsBatchConfig, *, force_rebuild: bool = False) -> Path:
    ...
    if (not needs_build) and pipeline_py.exists():
        ...
        required_tokens = [
            ...
            'GROMACS_TRIMER_FALLBACK_MAX_ATTEMPTS',
            '_fallback_attempt_configs',
            'chain_n{n_repeat}_build_quality.json',
            'GROMACS_ALLOW_CPU_FALLBACK',
            '_with_required_gpu_options',
            '_mdrun_log_confirms_gpu',
            '_production_gpu_extra',
            'GROMACS_PRODUCTION_DT_FS',
            '_run_mdrun_guarded',
            '[mdrun:lincs_instability]',
        ]
...
def run_batch_pipeline(manifest_df: pd.DataFrame, *, config: GromacsBatchConfig) -> pd.DataFrame:
    ...
    BATCH_PRODUCTION_DT_FS = float(CFG.get('production_dt_fs', 1.0))
    if BATCH_PRODUCTION_DT_FS <= 0:
        raise ValueError(f"Invalid BATCH_PRODUCTION_DT_FS={BATCH_PRODUCTION_DT_FS}")
    ...
    BATCH_PRODUCTION_BONDED_GPU = '1' if _as_bool(CFG.get('production_bonded_gpu', False), False) else '0'
    BATCH_PRODUCTION_LINCS_ITER = max(1, int(CFG.get('production_lincs_iter', 6)))
    BATCH_PRODUCTION_LINCS_ORDER = max(1, int(CFG.get('production_lincs_order', 12)))
    BATCH_PRODUCTION_LINCS_WARNANGLE = max(1, int(CFG.get('production_lincs_warnangle', 20)))
    BATCH_LINCS_ABORT_WARNINGS = max(1, int(CFG.get('lincs_abort_warnings', 25)))
    BATCH_ALLOW_CPU_FALLBACK = '1' if _as_bool(CFG.get('allow_cpu_fallback', False), False) else '0'
    ...
    def _classify_phase_failure(phase: str, stdout: str, last_stage: str = '') -> str:
        ...
        if phase == 'md' and (last_stage or '').startswith('md'):
            if '[mdrun:lincs_instability]' in text or 'lincs warning' in text:
                return 'lincs_instability'
            if (
                'cudafreehost' in text
                or 'cuda error' in text
                or 'invalidaddressspace' in text
                or 'memory violation' in text
                or 'segmentation' in text
            ):
                return 'gpu_memory_violation'
    ...
    def _atomtyping_recovery_env(error_type: str, attempt: int) -> dict:
        ...
        if et == 'acpype_atoms_too_close':
            ...
            env['GROMACS_ACPYPE_CHARGE_METHOD'] = 'gas'
            env['GROMACS_ACPYPE_ATOM_TYPE'] = 'gaff'
        elif et == 'polymer_trimer_fallback_failed':
            # The fallback can fail before chemistry/topology is tested because
            # pysoftk/RDKit created overlapping trimer/tetramer coordinates.
            # Retry atomtyping with stricter polymer probe generation instead of
            # forcing ACPYPE through a bad geometry.
            strict_level = max(1, attempt - 1)
            env['GROMACS_FAST_PYSOFTK'] = '0'
            env['GROMACS_PYSOFTK_SKIP_FINAL_LOCALOPT'] = '0'
            env['GROMACS_PYSOFTK_UFF_ITERS'] = str(min(6000, 2000 + 1000 * strict_level))
            env['GROMACS_PYSOFTK_LOCALOPT_STEPS'] = str(min(5000, 800 + 600 * strict_level))
            env['GROMACS_REBUILD_FROM_CHAIN_MOL'] = '1'
            env['GROMACS_ACPYPE_SKIP_REPAIR'] = '0'
            env['GROMACS_ACPYPE_MIN_DIST'] = f"{min(1.60, 1.15 + 0.10 * strict_level):.2f}"
            env['GROMACS_TRIMER_FALLBACK_STRICT_REBUILD'] = '1'
            env['GROMACS_TRIMER_FALLBACK_MAX_ATTEMPTS'] = '8'
            env['GROMACS_TRIMER_FALLBACK_MIN_DIST'] = f"{min(1.60, 1.10 + 0.10 * strict_level):.2f}"
            env['GROMACS_TRIMER_FALLBACK_SEED_OFFSETS'] = (
                '0,1009,2027,4099,7919,15401,22003,31013'
            )
        return env


    def _md_recovery_env(error_type: str, attempt: int) -> dict:
        env = {}
        et = (error_type or '').strip().lower()
        if et in ('lincs_instability', 'gpu_memory_violation', 'md_runtime_failure'):
            strict_level = max(1, attempt - 1)
            env['GROMACS_PRODUCTION_TCOUPL'] = 'v-rescale'
            env['GROMACS_PRODUCTION_TAU_T'] = '5'
            env['GROMACS_PRODUCTION_BONDED_GPU'] = '0'
            env['GROMACS_ALLOW_CPU_FALLBACK'] = '0'
            env['GROMACS_PRODUCTION_LINCS_ITER'] = str(min(10, 6 + strict_level))
            env['GROMACS_PRODUCTION_LINCS_ORDER'] = str(min(16, 12 + 2 * strict_level))
            env['GROMACS_PRODUCTION_LINCS_WARNANGLE'] = '20'
            env['GROMACS_LINCS_ABORT_WARNINGS'] = str(max(8, 25 - 5 * strict_level))
            if strict_level >= 2:
                env['GROMACS_PRODUCTION_DT_FS'] = '0.5'
            else:
                env['GROMACS_PRODUCTION_DT_FS'] = '1.0'
        return env
    ...
    def execute_phase_traj(
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            env['GROMACS_PRODUCTION_DT_FS'] = f'{BATCH_PRODUCTION_DT_FS:g}'
            ...
            env['GROMACS_PRODUCTION_LINCS_ITER'] = str(BATCH_PRODUCTION_LINCS_ITER)
            env['GROMACS_PRODUCTION_LINCS_ORDER'] = str(BATCH_PRODUCTION_LINCS_ORDER)
            env['GROMACS_PRODUCTION_LINCS_WARNANGLE'] = str(BATCH_PRODUCTION_LINCS_WARNANGLE)
            env['GROMACS_LINCS_ABORT_WARNINGS'] = str(BATCH_LINCS_ABORT_WARNINGS)
            env['GROMACS_ALLOW_CPU_FALLBACK'] = BATCH_ALLOW_CPU_FALLBACK
            ...
            if phase == 'md' and attempt > 1:
                recov = _md_recovery_env(last_error_type, attempt)
                env.update(recov)
                if recov:
                    print(f"[RECOVERY] md Traj_{traj_id} attempt {attempt}: {recov}")
                md_dir = run_dir / 'md'
                if md_dir.exists():
                    for stage_dir in md_dir.iterdir():
                        if not stage_dir.is_dir():
                            continue
                        stage_name = stage_dir.name
                        out_gro = stage_dir / f'{stage_name}.gro'
                        if stage_name.startswith('production') and (not out_gro.exists() or out_gro.stat().st_size <= 0):
    ...
    print(
        ...
        f'production_dt_fs={BATCH_PRODUCTION_DT_FS:g}',
        ...
        f'production_lincs_iter={BATCH_PRODUCTION_LINCS_ITER}',
        f'production_lincs_order={BATCH_PRODUCTION_LINCS_ORDER}',
        f'production_lincs_warnangle={BATCH_PRODUCTION_LINCS_WARNANGLE}',
        f'lincs_abort_warnings={BATCH_LINCS_ABORT_WARNINGS}',
        f'allow_cpu_fallback={BATCH_ALLOW_CPU_FALLBACK}',
