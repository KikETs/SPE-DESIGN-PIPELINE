...
def write_inventory(oof: pd.DataFrame, embeddings: np.ndarray) -> None:
    ...
    if generated_predictions.exists():
        generated_columns = set(pd.read_csv(generated_predictions, nrows=1).columns)
        generated_predictions_available = {
            "smiles",
...
def write_generated_candidate_notes(selection: pd.DataFrame) -> None:
    generated_predictions = TABLES / "weighted_generated_candidate_predictions.csv"
    if generated_predictions.exists():
        columns = set(pd.read_csv(generated_predictions, nrows=1).columns)
        if {"smiles", "weighted_pred_log10_conductivity"}.issubset(columns):
            return
...
def write_interpretation_reports(selection: pd.DataFrame, threshold_df: pd.DataFrame, topk_df: pd.DataFrame, tail_df: pd.DataFrame) -> None:
    ...
    if generated_predictions.exists():
        generated_columns = set(pd.read_csv(generated_predictions, nrows=1).columns)
        generated_available = {"smiles", "weighted_pred_log10_conductivity"}.issubset(generated_columns)
    generated_interpretation = (
        ...
        else "- Generated-candidate weighted predictions were not computed because candidate PolyBERT embeddings were unavailable."
    )
    generated_risk = (
        "- Weighted generated-candidate rankings are available, but the original baseline-based MD selection is retained."
        if generated_available
        else "- Weighted rankings for generated candidates remain unavailable until embeddings are generated."
    )
    generated_recommendation = (
        "- Supplementary: report the generated-pool weighted comparison separately from the OOF model-selection diagnostics."
        if generated_available
        else "- Limitations: state that weighted generated-candidate prediction requires candidate embeddings."
    )
