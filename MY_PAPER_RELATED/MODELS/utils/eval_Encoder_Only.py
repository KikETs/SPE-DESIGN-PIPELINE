...
def reconstruct_zmu(model, dataloader, vocab: dict, *,
    ...
    for _, dec_in, _, props in dataloader:
        dec_in = dec_in.to(device)
        props = props.to(device)

        with torch.no_grad():
            gen_in = torch.full((dec_in.size(0), 1), SOS, device=device, dtype=torch.long)
            done = torch.zeros(dec_in.size(0), dtype=torch.bool, device=device)
            out_tok = [[] for _ in range(dec_in.size(0))]

            for _ in range(max_len):
                logits = model(gen_in, props)[:, -1] / temperature
                next_tok = logits.argmax(-1, keepdim=True)
                gen_in = torch.cat([gen_in, next_tok], dim=1)
        ...
        for r_tok, o_tok in zip(dec_in.cpu().tolist(), out_tok):
