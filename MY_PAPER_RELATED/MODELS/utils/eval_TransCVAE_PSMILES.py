...
def eval_iwae_bound(
    ...
    valid_mask = torch.cat([valid_mask, torch.ones([B, 1], device=sm_enc.device)], dim=1).float()
    mask4 = valid_mask.unsqueeze(0).unsqueeze(-1)
    ...
    for k0 in range(0, K, chunk):
        ...
        log_px = ll.sum(-1).view(k, B)

        log_qz = (q.log_prob(z) * mask4).sum((2, 3))
        log_pz = p.log_prob(z).sum((2, 3))
    ...
    log_ws = torch.cat(log_ws, 0).double()
    log_ws_len = torch.cat(log_ws_len, 0).double()
    log_ws_latent = torch.cat(log_ws_latent, 0).double()
