...
class PI1MPretrainDataset(Dataset):
    """
    PI1M p-SMILES corpus -> pSMILES token LM pretraining dataset for CVAE.
    Returns tuple compatible with existing TransCVAE dataloader:
      (enc, dec_in, dec_out, properties)
    properties are dummy zeros with shape [N,1,1] during pretraining.
    """
    ...
    def _load_cache(self, data_path, meta_path):
        try:
            ...
            self.SMILES_enc = blob["SMILES_enc"].long()
            ...
            self.num_rows_read = int(meta.get("num_rows_read", len(self.SMILES_enc)))
            self.num_rows_valid = int(meta.get("num_rows_valid", len(self.SMILES_enc)))
    ...
    def _build_from_csv(self):
        ...
        for sm in tqdm(smiles_arr, desc="PI1M tokenize", leave=False):
            try:
                ...
                toks = split_psmiles_tokens(_psmiles_core(ps))
                if not toks:
                    dropped += 1
                    continue
                ps_tokens.append(toks)
            except Exception:
                dropped += 1
        ...
        self.num_rows_valid = len(ps_tokens)
        self.num_rows_dropped = dropped
        self.max_len = max(len(t) for t in ps_tokens) + 1

        corpus = [tok for seq in ps_tokens for tok in seq] + ["[SOS]", "[EOS]", "[PAD]"]
        self.vocab = {tok: i for i, tok in enumerate(sorted(set(corpus)))}
        self.vocab_size = len(self.vocab)
        self.num_data = len(ps_tokens)

        pad_idx = self.vocab["[PAD]"]
        sos_idx = self.vocab["[SOS]"]
        eos_idx = self.vocab["[EOS]"]

        enc = torch.full((self.num_data, self.max_len), pad_idx, dtype=torch.long)
        dec_in = torch.full_like(enc, pad_idx)
        dec_out = torch.full_like(enc, pad_idx)

        for i, seq in enumerate(tqdm(ps_tokens, desc="PI1M tensorize", leave=False)):
            seq_ids = [self.vocab[t] for t in seq]
            seq_len = len(seq_ids)

            enc[i, :seq_len] = torch.tensor(seq_ids, dtype=torch.long)
            dec_in[i, 0] = sos_idx
